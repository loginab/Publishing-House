...
/*
	public void testCommitTransaction()
	{
		...
		testCommit = true;
		//dbDAL.setAutoCommit(false);
		printForTestingRollback("Testing Commit transaction. \n Auto commit set to false.");
		
		// Trying to add valid data
		queryResponse = addVendorPurchaseRecord("SHIPPED","Turners Inc", "123454", "2");
				
		// If both query are successful then commit else rollback i.e.
		// 1) place a vendor purchase order
		// 2) If vendor order successful, then update the warehouse content:
		// 		a) If merchandise for which order is placed does not exist in warehouse content then add a new merchandise in warehouse.
		// 		b) If merchandise for which order is placed exists in warehouse then update the quantity in stock for that merchandise in
		// 			warehouse.
		// 3) If all three transactions (1, 2 a, 2b) are successful then commit the transaction else roll back the transaction

		if (queryResponse == dbDAL.DAL_SUCCESS) {
			printForTestingRollback("\n Both transactions 1. Placing vendor purchase order and 2. Updating warehouse content are successful. \n Now commiting records.");
			dbDAL.commitTransaction();
		} else {
			printForTestingRollback("\n Failure while executing one of the transaction among \n 1. Placing vendor purchase order and 2. Updating warehouse content. \n Rolling back both transactions.");
			dbDAL.rollbackTransaction();
		}
		
		// Set auto commit to true for later transactions
		//dbDAL.setAutoCommit(true);
		testCommit = false;
	}
	*/
